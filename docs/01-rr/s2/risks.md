...
| R2.6 | Concurrency | `quiche::Connection` is `!Send` due to BoringSSL's thread-local error queue; connections must be pinned to their birth thread via `sched_setaffinity` and a tokio task touching a connection from the wrong thread panics at runtime — invisible at compile time | [dependency-decisions](dependency-decisions.md) § 1.1 and § 3.1, [concurrency](../s1/catalogs/cross-cutting/concurrency/README.md) | High | High | P0 | Policy decision in [ADR-018](adr/018-quic-thread-affinity-enforcement.md); enforce wrappers and boundary constraints in S2.6 |
